...
int main(int argc, char **argv)
{
	...
	while (loops--) {
		len = slz_rfc1950_init(&strm, outbuf);

		ofs = 0;
		do {
			len += slz_rfc1950_encode(&strm, outbuf + len, buffer + ofs, (buflen - ofs) > BLK ? BLK : buflen - ofs, (buflen - ofs) > BLK);
			if (buflen - ofs > BLK) {
				totout += len;
				ofs += BLK;
				write(1, outbuf, len);
		...
		len += slz_rfc1950_finish(&strm, outbuf + len);
		totin += ofs;
		totout += len;
		write(1, outbuf, len);
	}
